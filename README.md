# Hopfield Network and Stimuli Repetition
Unpublished work by Joshua Oon Soo Goh [\[1\]](#1) in fulfillment of Psych 591 Neural Network Modeling course conducted in Spring 2007 at the Dept. of Psychology, University of Illinois at Urbana-Champaign, IL, USA, instructed by John Hummel. This work extends the Hopfield Network as implemented by John Hopfield [\[2\]](#2) by evaluating its performance with respect to stimuli repetition. The primary code is writtern in R ([HopfieldNetwork.R](/code/HopfieldNetwork.R)), which has recently been ported to a Jupyter Notebook [binary_autoencoder.ipynb](/code/binary_autoencoder.ipynb) with a [Google Colab instance](https://colab.research.google.com/drive/1vPtrVNpzqqMQak4g5tINBOwY5dM0Pqt5?usp=sharing)

**Online Full Lecture Videos (Youtube)**<br>
1. [Theory](https://youtu.be/O9tLk_FgZoE?si=I2SM75cU_iTG357n)
2. [Implementation](https://youtu.be/bA3zLusHSfs?si=QLUsrhnjdBcK1lco)
3. [Evaluation](https://youtu.be/J7YRHW07kg0?si=yjNpE5I6sDqTxVHy)

**Conceptual Notes (PDF)**<br>
1. [Blackboard](/docs/blackboard.pdf)

**Bibliography & Notes**<br>
1. <a id="1">Goh, J. O. S. (2007). Hopfield Network. *Unpublished*. [\[pdf\]](https://gitlab.com/joshuagoh/hopfield-network/-/blob/main/manuscript/Hopfield%20Network%20-%20Josh%20Goh.pdf?ref_type=heads)
2. <a id="2">Hopfield, J. J. (1982). Neural networks and physical systems with emergent collective computational abilities. Proceedings of the National Academy of Sciences USA, *79*, 2554-2558.
3. <a id="3">GIBMS 7015 Neural Networks Course, NTU COOL, National Taiwan University, [https://cool.ntu.edu.tw/courses/57182/pages/neural-networks](https://cool.ntu.edu.tw/courses/57182/pages/neural-networks)
4. [\[3\]](#3) uses the Hopfield Network Jupyter Notebook instance [binary_autoencoder.ipynb](/code/binary_autoencoder.ipynb)
